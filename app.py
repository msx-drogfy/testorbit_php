...
def register():
    if request.method == 'POST':
        ...
        cursor.execute("SELECT id FROM members WHERE username = %s OR email = %s", (username, email))
        ...
        try:
            ...
            cursor.execute("INSERT INTO members (username, email, passcode) VALUES (%s, %s, %s)",
...
def login():
    if request.method == 'POST':
        ...
        cursor.execute("SELECT id, username, passcode FROM members WHERE username = %s OR email = %s", (username_or_email, username_or_email,))
...
def solver():
    ...
    # Fetch the user ID
    cursor.execute("SELECT ID FROM members WHERE username = %s OR email = %s", (username, username,))
...
def exam_results():
    ...
    for student in participants:
        ...
        # Fetch username from the members table
        cursor.execute('SELECT username FROM members WHERE ID = %s', (user_id,))
