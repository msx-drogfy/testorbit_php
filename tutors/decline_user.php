...
require "userinfo.php";
    ...
    if ($conn) {
        ...
        if ($user_check && mysqli_num_rows($user_check) > 0) {
            ...
            $update = mysqli_query($conn, "UPDATE peer_group_members SET statusx='declined' WHERE group_key='$group_key' AND user_id='$user'");

            // Check if the update query was successful
            if ($update) {
                $name = get_user_info($user, "first_name")." ".get_user_info($user, "last_name");
                notify("$name has been declined", "warning");
