...
if ($questionId) {
    ...
    if ($question) {
        ...
        // Fetch user ID
        $userIdQuery = "SELECT ID FROM tutors WHERE username = ? OR email = ?";
        ...
        if ($isCorrect) {
            ...
            header("Location: exam_checkbox.php?q=$questionId&check=pass&select=$userAnswer"); // Redirect to the next question
        } else {
            notify("Option $userAnswerStr are not correct. Please try again.", "warning");
            header("Location: exam_checkbox.php?q=$questionId"); // Redirect to retry the same question
        }
    }
    $stmt->close();
}
